...
public:
	...
	BVH* right = NULL;
	static int count;
	BVH(glm::vec3 minv, glm::vec3 maxv,int coef,bool isroot = false, triangle trian = triangle())
	{
		minVertex = minv; maxVertex = maxv; coefficient = coef; isLeaf = isroot; t = trian; right = NULL; left = NULL;
	}
...
BVH* recursionBVH(vector<triangle> &triangleList, int begin, int end, xyz axis) {
	if (begin + 1 == end)
	{
		return new BVH(triangleList[begin].minTriangle(), triangleList[begin].maxTriangle(), BVH::count++,true, triangleList[begin]);
	}
	switch (axis)
	{
	case xAxis:
	...
	BVH* bvh = new BVH(minV, maxV,BVH::count++);
	switch (axis)
	{
	case xAxis:
...
void recursionWriteTexture(float *textureMat, float *triangleMat, BVH *root, int index, int &idx) {
	...
	recursionWriteTexture(textureMat, triangleMat, root->left, 3 * root->coefficient , idx);
	recursionWriteTexture(textureMat, triangleMat, root->right, 3 * root->coefficient, idx);
}
...
void createTexture(BVH* root, float *&textureMat, float *&triangleMat, int &bvhSize, int &triSize)
{
	...
	bvhSize = nubmerOfNodes(root) * 3 * 3;
